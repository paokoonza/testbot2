...
const myBot = ['ua1cb6e845fe8f2646fe8a5c5911841fa','ue5e311b6ad93ed970d78c9fc392141b6'];
...
\n\n# Gunakan bot dengan bijak ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 17 && this.stateStatus.salam == 1 && isAdminOrBot(operation.param2)) {//ada yang join
			...
			halobos.toType = 2;
			halobos.text = "สวัสดีครีบบอสยินดีตอนรับเข้ากลุ่ม !";
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "สวัสดีครับแอดมามีไรป่าวคับ: line://ti/p/~socool290 (~GoogleX)";
			this._client.sendMessage(0, halo);
        }
...
Link Download: "+idU.id+"\n";
        ...
        if(txt === '!ปิดบิน.' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === '!ปิดบิน.' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
				if(txt == '!key') {
		...
		if(txt == '0101') {//Jangan dicoba (gk ada efek)
            ...
            for (var i = 0; i < listMember.length; i++) {
                if(listMember[i].mid==param){
					let namanya = listMember[i].dn;
					seq.text = 'Halo @'+namanya+', สวีสดีครับยินดีตอนรับเข้ากลุ่มครับ^_^';
					let midnya = listMember[i].mid;
