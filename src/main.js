...
\n\n# BOT. LINE. BY:TUM ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 17 && this.stateStatus.salam == 1 && isAdminOrBot(operation.param2)) {//ada yang join
			...
			halobos.toType = 2;
			halobos.text = "Halo bos !, สวัสดีครับ group ini bos !";
...
Link Download: "+idU.id+"\n";
        ...
        if(txt === '!ปิด!' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === '!ปิด!' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
				if(txt == '!key') {
